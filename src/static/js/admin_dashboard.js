...
let editModal;
    ...
    async function confirmDeleteCategoryFromDropdown(){
        ...
        try {
            const res = await apiFetch('/api/delete_category',{
                method:'POST',
                body:JSON.stringify({category:categoryToDelete, force: false})
            });
            const data = await res.json();
            ...
            if (data.deleted) {
                ...
                document.getElementById('deleteCategoryWarningText').innerText = `This category has ${data.count} books. Delete category and all its books, or Cancel?`;
    ...
    async function executeCategoryDelete(force=true){
        if(!categoryToDelete) return;
        try {
            const res = await apiFetch('/api/delete_category',{
                method:'POST',
                body:JSON.stringify({category:categoryToDelete, force: force})
            });
    ...
    async function deleteUser(school_id) {
        if (!confirm(`Are you sure you want to delete user ${school_id}? This action cannot be undone.`)) return;
        try {
            const res = await apiFetch('/api/delete_member', {
                method: 'POST',
                body: JSON.stringify({ school_id })
            });
            const data = await res.json();
            if (data.success) {
                addHistory(`Deleted User: ${school_id}`);
                loadData(false);
            } else {
    ...
    function renderUsersList() {
        ...
        const yearFilter = document.getElementById('userYearFilter')?.value || 'all';
        ...
        let filtered = combined.filter(u => {
            const matchQuery = (u.name||'').toLowerCase().includes(query) || (u.school_id||'').toLowerCase().includes(query);
            const matchType = typeFilter === 'all' || u.type === typeFilter;
            const matchYear = yearFilter === 'all' || String(u.year_level || '') === yearFilter;
            const matchCourse = courseFilter === 'all' || (u.course || '') === courseFilter;
            return matchQuery && matchType && matchYear && matchCourse;
        });

        filtered.sort((a, b) => {
            let valA = a[_userSortCol] || '';
            let valB = b[_userSortCol] || '';
            if (typeof valA === 'string') valA = valA.toLowerCase();
            if (typeof valB === 'string') valB = valB.toLowerCase();
            if (valA < valB) return -1 * _userSortDir;
            if (valA > valB) return 1 * _userSortDir;
            return 0;
        });

        if (badge) badge.innerText = `${filtered.length} user${filtered.length !== 1 ? 's' : ''}`;

        tbody.innerHTML = filtered.map(u => {
            const isStudent = u.type === 'student';
            const yearStr = isStudent ? (u.year_level || '-') : '-';
            const courseStr = isStudent ? (u.course || '-') : '-';
            const contactStr = isStudent ? (u.phone_number || u.email || '-') : '-';
            const roleBadge = u.type === 'admin' ? '<span class="badge bg-danger">ADMIN</span>' : '<span class="badge bg-primary">STUDENT</span>';
            const defaultPhoto = isStudent ? 'avatar_owl.svg' : 'admin_avatar.svg';
            const removeBtn = u.type === 'admin' ? '-' : `<button class="btn btn-sm btn-outline-danger border-0" onclick="deleteUser('${u.school_id}')" title="Remove User"><i class="fas fa-trash"></i></button>`;

            return `
            <tr>
                <td class="ps-3">
                    <img src="/static/img/avatars/${u.photo || defaultPhoto}" class="rounded-circle shadow-sm" style="width:36px;height:36px;object-fit:cover;" onerror="this.src='/static/img/avatars/avatar_owl.svg'">
                </td>
                <td><code class="fw-bold" style="color:#e2e8f0;background:rgba(255,255,255,0.05);padding:3px 6px;border-radius:4px;">${u.school_id}</code></td>
                <td class="fw-bold" style="color:#f8fafc;">${u.name}</td>
                <td><span style="color:#cbd5e1;">${yearStr}</span></td>
                <td><span style="color:#cbd5e1;">${courseStr}</span></td>
                <td><span style="color:#cbd5e1;font-size:0.8rem;">${contactStr}</span></td>
                <td>${roleBadge}</td>
                <td><span class="badge" style="background:rgba(16,185,129,0.2);color:#34d399;border:1px solid rgba(16,185,129,0.35);">Active</span></td>
                <td class="text-center pe-3">${removeBtn}</td>
            </tr>`;
        }).join('') || '<tr><td colspan="9" class="text-center py-5" style="color:rgba(255,255,255,0.4);"><i class="fas fa-search d-block mb-2" style="font-size:2rem;"></i>No records found.</td></tr>';
    }
    ...
    function setQuickRegisterRole(role) {
        ...
        const sFields = document.getElementById('quickRegStudentFields');
        const cFields = document.getElementById('quickRegContactWrap');
        if (sFields) sFields.style.display = currentRole === 'student' ? 'block' : 'none';
        if (cFields) cFields.style.display = currentRole === 'student' ? 'block' : 'none';
        if (currentRole === 'student') updateQuickRegCourses();
    }

    function updateQuickRegCourses() {
        const level = document.querySelector('input[name="quickRegLevel"]:checked')?.value || 'college';
        const select = document.getElementById('quickRegCourse');
        if (!select) return;
        
        let filtered = [];
        if (Array.isArray(masterCourses)) {
            filtered = masterCourses.filter(c => c.school_level === level);
        }
        ...
        // Also update quickRegYear based on level if needed
        const yearSelect = document.getElementById('quickRegYear');
        if (yearSelect) {
            yearSelect.innerHTML = '<option value="">Select Year</option>';
            const maxYear = level === 'college' ? 4 : 4; // Or handle grade 7-10 for JHS
            if (level === 'jhs') {
                for (let i = 7; i <= 10; i++) yearSelect.innerHTML += `<option value="${i}">Grade ${i}</option>`;
            } else {
                for (let i = 1; i <= 4; i++) yearSelect.innerHTML += `<option value="${i}">${i}${i===1?'st':i===2?'nd':i===3?'rd':'th'} Year</option>`;
            }
        }
    }
    ...
    async function submitQuickRegister() {
        ...
        if (currentRole === 'student') {
            form.append('school_level', document.querySelector('input[name="quickRegLevel"]:checked')?.value || 'college');
            form.append('year_level', document.getElementById('quickRegYear')?.value || '');
            form.append('course', document.getElementById('quickRegCourse')?.value || 'N/A');
    ...
    function renderCourses() {
        const collegeBody = document.getElementById('collegeCoursesBody');
        const jhsBody = document.getElementById('jhsCoursesBody');
        if (collegeBody) {
            collegeBody.innerHTML = masterCourses.filter(c => c.school_level === 'college').map(c => `
                <tr>
                    <td class="fw-bold">${c.name}</td>
                    <td class="text-end">
                        <button class="btn btn-sm btn-outline-primary me-1" onclick="editCourse(${c.id}, '${c.name.replace(/'/g, "\\'")}', 'college')">Edit</button>
                        <button class="btn btn-sm btn-outline-danger" onclick="confirmDeleteCourse(${c.id}, '${c.name.replace(/'/g, "\\'")}')">Delete</button>
                    </td>
                </tr>
            `).join('') || '<tr><td colspan="2" class="text-center py-3 text-muted">No college courses found.</td></tr>';
        }
        if (jhsBody) {
            jhsBody.innerHTML = masterCourses.filter(c => c.school_level === 'jhs').map(c => `
                <tr>
                    <td class="fw-bold">${c.name}</td>
                    <td class="text-end">
                        <button class="btn btn-sm btn-outline-primary me-1" onclick="editCourse(${c.id}, '${c.name.replace(/'/g, "\\'")}', 'jhs')">Edit</button>
                        <button class="btn btn-sm btn-outline-danger" onclick="confirmDeleteCourse(${c.id}, '${c.name.replace(/'/g, "\\'")}')">Delete</button>
                    </td>
                </tr>
            `).join('') || '<tr><td colspan="2" class="text-center py-3 text-muted">No JHS courses found.</td></tr>';
        }
    }

    async function adminAddCourse(level) {
        const inputId = level === 'college' ? 'collegeCourseInput' : 'jhsCourseInput';
        const name = document.getElementById(inputId)?.value.trim();
        if (!name) return;
        try {
            const res = await apiFetch('/api/courses/', {
                method: 'POST',
                body: JSON.stringify({ name, school_level: level })
            });
            const data = await res.json();
            if (data.success) {
    ...
    async function editCourse(id, oldName, level) {
        ...
        try {
            ...
            if (data.success) {
                ...
                alert(data.message || 'Failed to edit course');
            }
        } catch(e) {
    ...
    async function confirmDeleteCourse(id, name) {
        try {
            ...
            if (data.deleted) {
                ...
                document.getElementById('deleteCourseWarningText').innerText = `This course has ${data.count} users. Delete course and keep users with N/A, or Cancel?`;
                document.getElementById('deleteCourseId').value = id;
                new bootstrap.Modal(document.getElementById('deleteCourseModal')).show();
            }
        } catch(e) {
            alert('Error checking course deletion');
        }
    }

    async function executeCourseDelete(force) {
