...
def api_delete_category(request):
    ...
    # Allow DELETE or POST method
    data = parse_json_body(request) if request.body else {}
    
    # Try to get from URL params if DELETE, otherwise JSON body
    cat = str(data.get('category', request.GET.get('category', ''))).strip()
    force = request.GET.get('force') == 'true' or data.get('force') is True
